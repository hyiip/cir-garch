...
variable = {
    "Leakage Ratio":{
        "panel": "D",
        "greek": r'$\sigma_x^2 / 4 \kappa \theta$',
        "label": "Leakage Ratio",
        "loc": "upper center",
        "nbins": 10,
        "png": "leakage.png",
...
inr[inr.columns[0]].plot(
    ax = axs , label="{} (left)".format(setting[region]["title"]), linewidth = 2.5,
    xticks = np.arange(0, len(inr)+1, 250.0), 
    title= "Panel {}\n{} - {}".format(variable[param]["panel"],variable[param]["greek"],setting[region]["title"]))
inr["S_U"].plot(ax = axs, linestyle="dotted", color = "darkgreen" ,label="Upper Bound (left)", linewidth = 1.5)
inr["Leakage Ratio"].plot(
    ax = secax , label="{} (right)".format(variable[param]["greek"]), linestyle="dashed", color = "red",  linewidth = 2)
