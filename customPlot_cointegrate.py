...
lplot2 =US_res["UST10YYIELD"].plot(
    ax = axs[1] , color = "brown",label="10Y US Treasury Yield (left)", linewidth = 1.5)
rplot2 =US_res["log_VIX"].plot(
    ax = secaxs[1] , linestyle="dashdot", color = "green",label="ln(VIX) (right)", linewidth = 1.5)
lplot3 =US_res["UST10YYIELD"].plot(
    ax = axs[2] ,color = "brown", label="10Y US Treasury Yield (left)", linewidth = 1.5)
#print(US_res["EPU"])
rplot3 =US_res["log_EPU"].plot(
    ax = secaxs[2] , linestyle="dashdot", color = "grey",label="ln(EPU) (right)", linewidth = 1.5)

axs[0].xaxis.set_tick_params(labelbottom=True)
axs[1].xaxis.set_tick_params(labelbottom=True)
axs[2].xaxis.set_tick_params(labelbottom=True)
axs[2].xaxis.label.set_visible(False)
pTitle = ["10Y GER Gov. Bond Yield", "10Y US Treasury Yield","10Y US Treasury Yield"]
sTitle = ["IT CDS", "ln(VIX)","ln(EPU)"]
